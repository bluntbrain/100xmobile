...
// individual tweet card - compact for multi-column layout
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            ...
            {user.is_blue_verified && (
              <svg viewBox="0 0 22 22" className="w-3.5 h-3.5 fill-[#1d9bf0] flex-shrink-0">
        ...
        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-[#71767b] flex-shrink-0">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
        </svg>
      </div>

      {/* text */}
      <p className="text-[#e7e9ea] text-[13px] leading-snug mb-2 whitespace-pre-wrap break-words">
...
export function TweetWall({ tweets }: TweetWallProps) {
  ...
  // responsive column count: 1 mobile, 2 sm, 3 md, 4 lg, 5 xl
  // we split into max 5 columns; CSS grid handles which ones show
  const maxCols = 5;
  ...
  return (
    <section className="mx-auto px-2 sm:px-4 pb-8">
      {/* mobile: single static column, no marquee */}
      <div className="sm:hidden space-y-3">
        {tweets.map((tweet) => {
          const data = tweetData.get(tweet.id);
          if (!data) return null;
          return <TweetCard key={tweet.id} tweet={data} url={tweet.url} />;
        })}
      </div>

      {/* sm+: marquee columns */}
      <div className="hidden sm:grid tweet-grid">
